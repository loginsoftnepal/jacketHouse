/*
  ...
  - Changed the type of `userId` on the `ForgetPasswordToken` table. No cast exists, the column would be dropped and recreated, which cannot be done if there is data, since the column is required.
  - Changed the type of `userId` on the `Session` table. No cast exists, the column would be dropped and recreated, which cannot be done if there is data, since the column is required.
  - Changed the type of `userId` on the `UserAvatar` table. No cast exists, the column would be dropped and recreated, which cannot be done if there is data, since the column is required.
...
-- DropForeignKey
ALTER TABLE "ForgetPasswordToken" DROP CONSTRAINT "ForgetPasswordToken_userId_fkey";
...
-- AlterTable
ALTER TABLE "ForgetPasswordToken" DROP COLUMN "userId",
ADD COLUMN     "userId" INTEGER NOT NULL;
...
-- AlterTable
ALTER TABLE "UserAvatar" DROP COLUMN "userId",
ADD COLUMN     "userId" INTEGER NOT NULL;
