...
function NavProfile({ session }: NavProfileType) {
  return (
    <DropdownMenu>
      <DropdownMenuTrigger>
          ...
          <AvatarImage
            src={
              session && session?.user?.image
                ? `${process.env.NEXT_PUBLIC_SERVER_URL}/api/file/avatar?filename=${session.user.image}`
                : ''
            }
            alt="@shadcn"
          />
      ...
      <DropdownMenuContent>
        ...
        <DropdownMenuItem>
          <Button
            className="w-full h-5 py-0 px-0 flex justify-start"
            variant={'ghost'}
            onClick={() => signOut()}
          >
